...
| 能力 | 适用条件 | 边界 |
| --- | --- | --- |
| CodeGraph | 需要从明确问题追踪跨文件调用链、依赖或架构关系 | 先用 `rg` 缩小入口；图结果必须抽查真实文件，不上传代码。 |
| Codebase Memory | 长期维护的大仓库，需要跨任务复用本地索引、架构检索、路径追踪或变更检测 | 先确认项目路径和索引新鲜度；不要记录完整聊天、客户数据或私有日志。 |

两者都是可选本地 MCP，不是默认插件依赖。一次任务只选一个主索引；当结果命中不足、索引陈旧或另一工具有明确能力增量时才切换。
